...
var _default = api => {
  api.describe({
    ...
    config: {
      schema: joi => {
        return joi.object({
          forceInitial: joi.boolean().description('remove window.g_initialProps in html, to force execing Page getInitialProps  functions'),
  ...
  api.onGenerateFiles( /*#__PURE__*/_asyncToGenerator(function* () {
    var _api$config$ssr, _api$config$ssr2, _api$config$ssr3;
  ...
  api.modifyHTMLChunks((memo, opts) => {
    ...
    if (api.config.dynamicImport && api.env !== 'development' && opts.chunks) {
      ...
      const chunkArr = [];
      const chunkName = (0, _utils().routeToChunkName)({
        route,
        cwd: api.cwd
      });
  ...
  api.modifyConfig(config => {
    ...
    config.devServer.writeToDisk = filePath => {
      var _api$config, _api$config$manifest;

      const manifestFile = ((_api$config = api.config) === null || _api$config === void 0 ? void 0 : (_api$config$manifest = _api$config.manifest) === null || _api$config$manifest === void 0 ? void 0 : _api$config$manifest.fileName) || 'asset-manifest.json';
  ...
  api.modifyDevHTMLContent( /*#__PURE__*/function () {
    var _ref3 = _asyncToGenerator(function* (defaultHtml, {
      req
    }) {
      var _api$config2;

      // umi dev to enable server side render by default
      const _ref4 = ((_api$config2 = api.config) === null || _api$config2 === void 0 ? void 0 : _api$config2.ssr) || {},
            stream = _ref4.stream,
            _ref4$devServerRender = _ref4.devServerRender,
            devServerRender = _ref4$devServerRender === void 0 ? true : _ref4$devServerRender;

      const serverPath = path().join(api.paths.absOutputPath, _constants.OUTPUT_SERVER_FILENAME); // if dev clear cache
  ...
  api.chainWebpack( /*#__PURE__*/function () {
    var _ref5 = _asyncToGenerator(function* (config, opts) {
      const paths = api.paths;
      const type = opts.type;
      const serverEntryPath = path().join(paths.absTmpPath, 'core/server.ts');
      config.plugin('progress').tap(([args]) => [_objectSpread({}, args, {
        name: 'Client'
      })]);
      ...
      if (type === _types().BundlerConfigType.ssr) {
        ...
        config.output.filename(_constants.OUTPUT_SERVER_FILENAME).libraryExport('default').chunkFilename('[name].server.js').libraryTarget('commonjs2'); // disable *.server.chunk.js, routes avoid dynamic and require in different mode.
