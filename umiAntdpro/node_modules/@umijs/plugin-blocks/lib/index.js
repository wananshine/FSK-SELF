...
const signale = _umi().utils.signale,
...
var _default = api => {
  ...
  function _block() {
    _block = _asyncToGenerator(function* (args = {}, opts = {}) {
      ...
      switch (args._[0]) {
        case 'clear':
          yield (0, _blockSdk().clearGitCache)({
            dryRun: args.dryRun
          });
          break;

        case 'add':
          retCtx = yield (0, _blockSdk().addBlock)(_objectSpread({}, args, {
            url: args._[1]
          }), opts, api);
          break;

        case 'list':
          retCtx = yield (0, _blockSdk().getDefaultBlockList)(args, blockConfig, api);
          break;

        default:
          throw new Error(`Please run ${chalk.cyan.underline('umi help block')} to checkout the usage`);
      }
