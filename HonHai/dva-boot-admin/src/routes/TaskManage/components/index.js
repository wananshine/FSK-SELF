...
export default class extends BaseComponent {
  ...
  render() {
      ...
      const columns = [
          ...
          {
              title: '执行状态',
              dataIndex: 'runResult',  //0：任务开始,1：任务完成,2：走出储位,3：任务失败,4:任务取消,5.任务未执行
              render: (tags) => (
                  <span>
                      { [null, ''].includes(tags) && <Tag>{tags}</Tag> }
                      { tags && [0, '0'].includes(tags) && <Tag color="#ffc107">任务开始</Tag> }
                      { tags && [1, '1'].includes(tags) && <Tag color="#04bd58">任务完成</Tag> }
                      { tags && [2, '2'].includes(tags) && <Tag color="#2db7f5">走出储位</Tag> }
                      { tags && [3, '3'].includes(tags) && <Tag color="#f5222d">任务失败</Tag> }
                      { tags && [4, '4'].includes(tags) && <Tag color="#fa541c">任务取消</Tag> }
                      { tags && [5, '5'].includes(tags) && <Tag color="#a0a0a0">任务未执行</Tag> }
                  </span>
