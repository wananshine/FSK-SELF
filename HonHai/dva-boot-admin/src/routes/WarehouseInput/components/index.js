...
export default class extends BaseComponent {
    ...
    onEdit = record =>{
        this.setState({
            ...
            formData: (record && {
                ...
                "taskTypeId": Number(record.taskTypeId) || '',
                // "taskTypeCode": record.taskTypeCode || null
        ...
        dispatch({
            ...
            this.setState({
                ...
                formData: {
                    ...this.state.formData,
                    [key]: val,
                    taskTypeId: Number(res.tasktypeId),
                    taskTypeCode: res.tasktypeName
                    // taskTypeCode: res.tasktypeId
                }
            });
        });
    ...
    handleOptionChange(key1, key2, option){
        this.setState({
            ...this.state,
            formData: {
                ...this.state.formData,
                [key1]: option.taskTypeId,
                [key2]: option.taskTypeCode,
            }
        })
    }
    ...
    render() {
        ...
        const { pageData, pageNum, pageSize, pNameData, lineData, locOptions, employees, taskOptions } = warehouse;
                              ...
                              <Form ref='ModalForm'>
                                  ...
                                  <Form.Item label="Line：" {...formItemLayout}>
                                      <Select defaultValue={formData.line}  onChange={(val, option)=>{this.handleLineChange(val, 'line', option)}} placeholder="请选择Line">
                                          {
                                              lineOptions && lineOptions.map((v, i)=>{
                                                  return <Option key={i.toString() + `${v.dictCode}`} dataCode={v.dictCode} value={v.dictCode}>产线：{v.dictLabel}</Option>
                                              })
                                          }
                                      </Select>
                                  </Form.Item>
                                  ...
                                  <Form.Item label="任务类型：" {...formItemLayout}>
                                      {/* onChange={(val)=>{this.handleChange(val, 'taskTypeId')}} */}
                                      <Select value={formData.taskTypeId} disabled onChange={(val, option)=>{this.handleOptionChange('taskTypeId', 'taskTypeCode',option)}}  placeholder="请选择任务类型">
                                          {
                                              taskOptions && taskOptions.map((v, i)=>{
                                                  return <Option key={v.dictCode} value={v.dictCode} taskTypeCode={v.dictValue} taskTypeId={v.dictCode}>{v.dictLabel}</Option>
                                              })
                                          }
                                      </Select>
                                  </Form.Item>
